...
# Works, g2g
# Same as "Normal_Reg_v1" but with a log transformation
...
#set.seed(2182021)
    ...
    XB <- X %*% Beta
    dat$XB <- as.vector(XB)
    dat$Y <- rlnorm(n = N, meanlog = dat$XB, sd = sqrt(sigma2))
...
X <- model.matrix( ~ Group , data = dat) 
Y.hat <- X %*% Beta.hat
sigma2.hat <- sum((Y.hat - log(dat$Y))^2)/(N-2)  # N-p-1, where p is number of predictors
